...
//                spi_xfer[2].delay_usecs = 0;
//
//		if (data != 0)
//		{
//			fprintf( stderr, "Setting up TX as well with len %d\n", data_len );
//
//			if (direction == DIRECTION_SEND)
...
//	}
	...
	if (mode == MODE_SPI)
	{
		//unsigned tx_len, rx_len = 0;

		//if ( direction == DIRECTION_SEND )
		//{
		//   tx_len = oplen + data_len + ret_len;
		//}

		uint8_t tx_buffer[ ( oplen + data_len ) ];
		uint8_t rx_buffer[ ( oplen + data_len ) ];
		memset( tx_buffer, 0, sizeof(tx_buffer) );
		memset( rx_buffer, 0, sizeof(rx_buffer) );

		// Prepare TX buffer
		memcpy( tx_buffer, cmd_buffer, oplen );

		if ( direction == DIRECTION_SEND )
		{
			fprintf( stderr, "Appending data with size %d\n", data_len );
			memcpy( &tx_buffer[oplen], data, data_len );
		}
                fprintf( stderr, "\nData: \n" );
		...
		memset( &spi_xfr, 0, sizeof(spi_xfr) );

		spi_xfr.rx_buf = (unsigned long)rx_buffer;
                spi_xfr.tx_buf = (unsigned long)tx_buffer;
                spi_xfr.len = sizeof(tx_buffer);
                spi_xfr.delay_usecs = 0;

	        status = ioctl(dev_fd, SPI_IOC_MESSAGE(1), &spi_xfr);

		memcpy( data, &rx_buffer[oplen], data_len );

#if DEBUG2
		fprintf( stderr, "\nSending: \n" );
		for ( unsigned i = 0; i < (oplen+data_len); ++i )
		{
			fprintf ( stderr, "0x%02x, ", tx_buffer[i] );
		}

		fprintf( stderr, "\nGot: \n" );
                for ( unsigned i = 0; i < (oplen+data_len); ++i )
                {
                        fprintf ( stderr, "0x%02x, ", rx_buffer[i] );
                }
...
int open_device(char *dev_name, int mode, int addr)
{
	...
	i2c_addr = addr;
        ...
        DEBUG(fprintf(stderr, "Open device %s\n, address: %d", dev_name, i2c_addr ));
	...
	int ret = ioctl(dev_fd, SPI_IOC_WR_BITS_PER_WORD, &bits);
	if (ret == -1)
		fprintf(stderr,"can't set bits per word");

	ret = ioctl(dev_fd, SPI_IOC_RD_BITS_PER_WORD, &bits);
	if (ret == -1)
		fprintf(stderr,"can't get bits per word");

	ret = ioctl(dev_fd, SPI_IOC_WR_MODE, &mod);
	if (ret == -1)
		fprintf(stderr,"can't set spi mode");

	ret = ioctl(dev_fd, SPI_IOC_RD_MODE, &mod);
	if (ret == -1)
		fprintf(stderr, "can't set spi mode");

	uint32_t speed = 1000000;

	ret = ioctl(dev_fd, SPI_IOC_WR_MAX_SPEED_HZ, &speed);
	if ( ret == -1 )
		DEBUG(fprintf(stderr, "Failed to reduce SPI clock \n"));

        ret = ioctl(dev_fd, SPI_IOC_RD_MAX_SPEED_HZ, &speed);
        if ( ret == -1 )
                DEBUG(fprintf(stderr, "Failed to reduce SPI clock \n"));
